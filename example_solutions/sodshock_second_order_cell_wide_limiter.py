...
################################################################################
# @file sodshock_second_order_cell_wide_limiter.py
#
# @brief Second order finite volume solution for the 1D Sod shock problem, using
# a cell wide slope limiter with variable tolerance factor.
#
...
# parameter that determines how diffuse the cell wide slope limiter is
# the value should be larger than or equal to 0.5 for a second order accurate
# scheme; larger values are more accurate, but also more unstable
BETA = 0.5
...
##
# @brief Apply a cell wide slope limiter to the gradient of the given quantity.
#
# @param grad_phi_i Unlimited gradient of the quantity.
# @param x_i Position of the center of the cell for which the gradient is
# computed.
# @param phi_i Value of the quantity at the position of the center of the cell.
# @param x_im1 Position of the cell to the left of the active cell.
# @param phi_im1 Value of the quantity at the position of the center of the left
# neighbouring cell.
# @param x_ip1 Position of the cell to the right of the active cell.
# @param phi_ip1 Value of the quantity at the position of the center of the
# right neighbouring cell.
# @return Slope limit factor: the gradient will be slope limited if multiplied
# with this factor.
##
def slope_limit(grad_phi_i, x_i, phi_i, x_im1, phi_im1, x_ip1, phi_ip1):
  dxmin = 0.5 * (x_im1 - x_i)
  dxplu = 0.5 * (x_ip1 - x_i)
  phi_max = max(phi_im1, phi_ip1)
  phi_min = min(phi_im1, phi_ip1)
  phi_im1_ext = phi_i + dxmin * grad_phi_i
  phi_ip1_ext = phi_i + dxplu * grad_phi_i
  ...
  if not phi_i == phi_min_ext:
    ...
    alpha_min = 1.
  return min(1., BETA * min(alpha_max, alpha_min))
...
for i in range(numstep):
  ...
  for icell in range(numcell):
    ...
    # apply the cell wide slope limiter
    alpha_density = slope_limit(gradient_density, xcell, densitycell,
                                xmin, densitymin, xplu, densityplu)
    alpha_velocity = slope_limit(gradient_velocity, xcell, velocitycell,
                                 xmin, velocitymin, xplu, velocityplu)
    alpha_pressure = slope_limit(gradient_pressure, xcell, pressurecell,
                                 xmin, pressuremin, xplu, pressureplu)  

    cells[icell]._gradient_density = alpha_density * gradient_density
    cells[icell]._gradient_velocity = alpha_velocity * gradient_velocity
    cells[icell]._gradient_pressure = alpha_pressure * gradient_pressure
...
pl.savefig("sodshock_second_order_cell_wide_limiter_beta_0_5.png")
